...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        with self.voiceover(text="Imagine we have a traveling salesperson - tasked with selling your products in various cities."):
            ...
            salesperson = ImageMobject("./src/pics/Salesman.png").scale(0.3)
        ...
        with self.voiceover(text="Alex has to sell products in various cities and wants to take the shortest route to save time. But how does Alex figure out the best route? Let's dive in and help Alex solve this puzzle."):
            ...
            svg_object = SVGMobject("./src/pics/world.svg").scale(3).set_color(WHITE)
            self.add(svg_object)
            self.add(salesperson)
        ...
        with self.voiceover(text="Let's say he needs to visit 4 cities."):
            ...
            warehouse_png = "./src/pics/warehouse.png"
            points0 = [
...
#          # create plain graph
        ...
        with self.voiceover(text="The graph is complete, this means there's a direct path from every city to every other city."):
            ...
            point_1.next_to(graph_with_labels, RIGHT, buff=1).shift(UP)
...
# # -----------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        with self.voiceover(text="Another way of solving the TSP is with the help of the branch and bound method. Let's start with a simple TSP example. Again we have a set of cities and need to find the shortest possible route visiting each city exactly once."):
        ...
        # create tree
        with self.voiceover(text="The Branch-and-Bound method begins by constructing a tree of all possibilities. First we need a Graph. Let's use the same graph and start at node one. "):
        ...
        with self.voiceover(text="Now we have to look at the next possible nodes. In this case we have four options. We can go to node 2, 3, 4 or 5. "):
        ...
        with self.voiceover(text="At the same time we can transform the graph on the right to a tree with node one as the root. This tree will show all the possible routes that the salesperson can use. As explained, starting with node one the next possible nodes could be node 2, 3, 4 or 5."):
        ...
        with self.voiceover(text="This would be the first version of the tree. Now we have to take a look at the next steps. Let's say we choose node 2 as the second node to travel to."):
        ...
        with self.voiceover(text="Starting from node 2, the next options would be node 3, 4 or 5."):
        ...
        with self.voiceover(text="Now we can create the next step at the tree. Again the next possible nodes to travel to would be node 3, 4 and 5. "):
        ...
        with self.voiceover(text="Let's continue creating one possible route. We decide to travel to node 5 as our next city. "):
        ...
        with self.voiceover(text="Starting from node 5, there are two cities left that have not been visited yet. Node 3 and 4."):
        ...
        with self.voiceover(text="Let's go back to the tree and add the last two options, node 3 and 4"):
        ...
        with self.voiceover(text="This time we choose node 3 as the next city."):
        ...
        with self.voiceover(text="As you can see the last city, that we have not visited is city 4."):
        ...
        with self.voiceover(text="Let's also add node 4 in the tree. Now we have visited every city."):
        ...
        with self.voiceover(text="At the end we have to travel to the city where we started the tour. Let's finish our route by traveling back to city 1."):
        ...
        with self.voiceover(text="Now we can also finish our route on the tree by adding node 1 as our last node."):
...
# # -----------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        with self.voiceover(text="As you can see this is one possible route and the tree is not complete. Let's create the complete tree."):
...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # Ending
        bf = ImageMobject("./src/pics/brute_force.png").scale(0.3).move_to(LEFT * 3 + UP * 2)
        bab = ImageMobject("./src/pics/bab.png").scale(0.3).move_to(RIGHT * 3 + UP * 2)
        ch = ImageMobject("./src/pics/christofides.png").scale(0.3).move_to(LEFT * 3 + DOWN * 2)
        knn = ImageMobject("./src/pics/knn.png").scale(0.3).move_to(RIGHT * 3 + DOWN * 2)
        ...
        with self.voiceover(text="Don't forget to subscribe and hit the bell so you won't miss our next adventure. Until next time, stay curious and take care. Bye!"):
            ...
            like = ImageMobject("./src/pics/like.jpg")
    ...
    def asymmetric(self):
        ...
        with self.voiceover(text="The TSP is called asymmetrical if there are two edges between every node and they don't have the same weight. As you can see the graph is then directed. This is way more accurate to the real world, but this is also more complex to solve then the symmetrical. In this video we will only show you ways of solving the symmetrical TSP."):
            ...
            # Shift labels of the inner edges
            (1, 0): (RIGHT+DOWN) * 0.3,
            (2, 1): (DOWN + LEFT) * 0.3,
            (3, 2): (UP + LEFT) * 0.3,
            (4, 3): UP * 0.3,
...
if __name__ == "__main__":
    ...
    os.system(f"manim --disable_caching -pqh --media_dir {media_path} {__file__} TSP")
